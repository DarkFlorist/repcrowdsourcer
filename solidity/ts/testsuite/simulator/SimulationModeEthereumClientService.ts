...
export const simulateEstimateGas = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, simulationState: SimulationState | undefined, data: PartialEthereumTransaction, blockDelta: number): Promise<EstimateGasError | { gas: bigint }> => {
	...
	try {
		...
		const gasSpent = lastResult.gasUsed * 130n * 64n / (100n * 63n) // add 25% * 64 / 63 extra  to account for gas savings <https://eips.ethereum.org/EIPS/eip-3529>
...
export const isValidMessage = async (params: SignMessageParams, signingAddress: EthereumAddress) => {
	try {
		await simulatePersonalSign(params, signingAddress)
		return true
	} catch(e) {
		console.error(e)
		return false
