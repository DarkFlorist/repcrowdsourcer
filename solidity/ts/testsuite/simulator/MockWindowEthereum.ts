		...
		request: async (unknownArgs: unknown): Promise<any> => {
			...
			switch(args.method) {
				...
				case 'eth_sendTransaction': {
					...
					const transaction = await formEthSendTransaction(ethereumClientService, undefined, simulationState, blockDelta, args.params[0].from, args)
					...
					const lastTx = simulationState.blocks.at(-1)?.simulatedTransactions.at(-1)?.ethSimulateV1CallResult
					if (lastTx?.status === 'failure') throw new Error(lastTx.error.message)
